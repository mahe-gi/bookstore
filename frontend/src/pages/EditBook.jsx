...
function CreateBooks() {
  ...
  useEffect(()=>{
    if(id){
      axios.get(`http://localhost:3000/api/books/${id}`).then((res)=>{
        const {title,author,publishedYear} = res.data;
        setTitle(title);
        setAuthor(author);
  ...
  const handleEditBook = ()=>{
    ...
    setLoading(true);
    axios.put(`http://localhost:3000/api/books/${id}`,data).then((res)=>{
      setLoading(false);
      navigate(`/`);
  ...
  return (
    <div className='p-4 '>
      ...
      <div className=' flex flex-col border-2 border-sky-400  rounded-xl w-[600px] px-4 mx-auto'>
        ...
        <button className=' p-2 bg-sky-300 m-8 ' onClick={handleEditBook}>Save Book
